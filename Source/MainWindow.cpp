	...
	switch (msg->what) {
		...
		case kOpenPreferencesMessage:
		{
			if (fPreferencesWindow == NULL) {
				fPreferencesWindow
					= new PreferencesWindow(Frame(), this,
		...
		case kToggleDeskbarReplicantMessage:
		{
			...
			if (!deskbar.HasItem("ForecastDeskbarView")) {
				...
				be_roster->GetAppInfo(
					"application/x-vnd.przemub.Weather", &info);

				status_t result = deskbar.AddItem(&info.ref);
				if (result != B_OK) {
					BString errorMessage
						= B_TRANSLATE("Unable to create a deskbar replicant. "
									  "The error is: \"");
...
MainWindow::AboutRequested()
{
	...
	about->AddDescription(B_TRANSLATE(
		"An open source weather app showing a forecast for the next 5 days."));
